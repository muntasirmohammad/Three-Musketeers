...
public class Board {
    ...
    public Board(String boardFilePath) {
        if(checkBoardSize(boardFilePath) == 7) {
        	size = 7;
    ...
    public int checkBoardSize(String boardFilePath) {
   	 File file = new File(boardFilePath);
        Scanner scanner = null;
        try {
            scanner = new Scanner(file);
        } catch (FileNotFoundException e) {
            System.err.printf("File at %s not found.", boardFilePath);
            System.exit(1);
        }
    ...
    public void setWinner(Piece.Type winner){
    	this.winner = winner;
    }
     ...
     * @param move the move that was undid previously, which needs to be redone.
     */
    public void redoMove(Move move) {
        ...
        toCell.setPiece(move.fromCell.getPiece());
    	changeTurn();
    }
    ...
    public String toString() {
    	if(size == 5) {
    		StringBuilder boardStr5 = new StringBuilder("  | A B C D E\n");
    		boardStr5.append("--+----------\n");
    		for (int i = 0; i < size; i++) {
    			boardStr5.append(i + 1).append(" | ");
    			for (int j = 0; j < size; j++) {
    				Cell cell = board[i][j];
    				boardStr5.append(cell).append(" ");
    			}
    			boardStr5.append("\n");
    		}
    		return boardStr5.toString();
    	}
    	else {
    		StringBuilder boardStr7 = new StringBuilder("  | A B C D E F G\n");
    		boardStr7.append("--+--------------\n");
    		for (int i = 0; i < size; i++) {
    			boardStr7.append(i + 1).append(" | ");
    			for (int j = 0; j < size; j++) {
    				Cell cell = board[i][j];
    				boardStr7.append(cell).append(" ");
    			}
    			boardStr7.append("\n");
    		}
    		return boardStr7.toString();
    	}
    }
